app.post('/api/extract', upload.single('file'), async (req, res) => {
    ...
    if (req.body) {  // Przypisac req.body do const body = req.body
        const file = req.file;
        // Zamiast powtarzac req.body uzyc const body 
        ...
        if (requestID && project && idUser && user) {
            ...
            if (user.role === 'ADVISOR' || user.role.indexOf('ADVISOR') > -1) // czy tutaj nie wystarczylby jeden z tych warunkow? Wydaje mi sie, ze sa tozsame
                                                                                // lub moze lepiej dac warunek: if (user.role && user.role === 'ADVISOR') ?
            ...
            if (project === 'inkasso' && config.projects.hasOwnProperty(project) && file) {
                const hashSum = crypto.createHash('sha256'); // stala hashSum nie jest nigdzie pozniej wykorzystywana - moze jest tutaj w takim razie niepotrzebna i mozna ja usunac
                ...
                let sent = true; // ta zmienna tez jest nieuzywana - mozna usunac?
                ...
                for (let i = 0; i < debtCollectors.length ; i++) {
                    ...
                    if (!!(await db.setUserRequestKey(requestKey, idUser))
                        && !!(await db.setUserCollectorRequestKey(requestKey, idUser, idCollector))) {
                            ...
                            // Stworzylabym funkcje, ktora zwraca object o ponizszej struktorze np. const configure = (args) => { function body; return config} 
                            // i wtedy const sendConfig = configure(wszystkie wartosci i objekt params)
                        /* prepare email */
                        const sendConfig = {
                            sender: config.projects[project].email.sender, // config.projects[project].email - to się powtarza więc mozna ewentualnie dac do consta wczesniej
                        ...
                        /* send email */
                        const resp = await email.send(sendConfig, config.projects[project].email.apiKey); // skorzystac z consta utworzonego wczesniej dla config.projects[project].email
                ...
                // Skorzystac z funkcji configure - const summaryConfig = configure(wszystkie wartosci i objekt params)
